...
export const searchCKANDataset = async (
	...
	try {
		...
		const res = await CKANPackageService.get("search", data, {
			Authorization: apiToken,
		});
...
export const getCKANDatasetAutocomplete = async (
	...
	try {
		const apiToken = await getUserAPIToken(username);
		console.log("ckan - searchCKANDataset - apiToken: ", apiToken);
		const res = await CKANPackageService.get("autocomplete", data, {
			Authorization: apiToken,
		});
...
export const requestAPIToken = async (
	...
	try {
		const res = await CKANUserService.create("api_token_create", {
			user: username,
			name: uuid4(),
		});
