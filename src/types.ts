...
export type CKANAPICapacity = 'member' | 'editor' | 'admin' | 'public' | 'private' | 'None';
...
export type CKANAPIRelationship = {
  ...
  type: 'depends_on' | 'dependency_of' | 'derives_from' | 'has_derivation' | 'links_to' | 'linked_from' | 'child_of' | 'parent_of', // (string) – the type of the relationship, one of 'depends_on', 'dependency_of', 'derives_from', 'has_derivation', 'links_to', 'linked_from', 'child_of' or 'parent_of'
...
export type CKANAPIPackageAutocompleteItem = {
  name: string, title: string, match_field: string, match_displayed: string
};
...
export interface CKANAPIUserListProps {
  ...
  order_by: 'id' | 'name' | 'fullname' | 'display_name' | 'created' | 'about' | 'sysadmin' | 'number_created_packages' // (string) – which field to sort the list by(optional, default: 'display_name').Users can be sorted by 'id', 'name', 'fullname', 'display_name', 'created', 'about', 'sysadmin' or 'number_created_packages'.
...
export interface CKANAPIUserhowProps {
  ...
  user_obj: CKANAPIUser //(user dictionary) – the user dictionary of the user(optional)
