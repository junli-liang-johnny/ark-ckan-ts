...
export class CKANUserService {
  static get(type: 'list' | 'show' | 'api_token_list', data: Partial<CKANAPIUserhowProps | CKANAPIUserListProps | CKANAPIUserTokenProps>): Promise<CKANAPIUserGetResponse | CKANAPIUserShowResponse | CKANAPIUserTokenResponse> {
    return new Promise((resolve, reject) => {
      ...
      if (type === 'api_token_list')
        ...
        url = `${CKANAPIBase.BASE_CKAN_API_URL}/user_show?${new URLSearchParams(data as any).toString()}`;
      else
        url = `${CKANAPIBase.BASE_CKAN_API_URL}/user_list?${new URLSearchParams(data as any).toString()}`;
      ...
      fetch(url, { 'headers': { 'Authorization': CKANAPIBase.API_KEY } })
        ...
        .then(res => {
          ...
          if (type === 'list')
            ...
            return resolve(res as CKANAPIUserTokenResponse)
        })
        .catch(err => {
  ...
  static create(type: 'user_create' | 'api_token_create', data: CKANAPIUserCreateProps | CKANAPIUserTokenCreateProps): Promise<CKANAPIUserShowResponse | CKANAPIUserTokenCreateResponse> {
    return new Promise((resolve, reject) => {
      ...
      if (type === 'user_create')
        ...
        url = `${CKANAPIBase.BASE_CKAN_API_URL}/api_token_create?${new URLSearchParams(data as any).toString()}`
      ...
      fetch(url, {
        ...
        .then(res => {
          if (type === 'user_create')
            resolve(res as CKANAPIUserShowResponse);
          else
            resolve(res as CKANAPIUserTokenCreateResponse);
        })
        .catch(err => {
  ...
  static remove(type: 'user_remove' | 'api_token_revoke', data: CKANAPIUserRemoveProps | CKANAPITokenRevokeProps): Promise<CKANAPIResponse> {
    return new Promise((resolve, reject) => {
      ...
      if (type === 'user_remove') {
        url = `${CKANAPIBase.BASE_CKAN_API_URL}/user_delete?${new URLSearchParams(data as any).toString()}`
